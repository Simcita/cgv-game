...
export async function createChildBedroom({
  ...
  return new Promise((resolve, reject) => {
    ...
    loader.load(url, (gltf) => {
      ...
      try {
        // Add a hemisphere light for soft ambient illumination (reduced for intense shadows)
        const hemi = new THREE.HemisphereLight(0xffffff, 0x444444, 0.4);
        hemi.position.set(0, 50, 0);
        roomGroup.add(hemi);

        // Directional light to create clear shadows inside the room (increased for stronger contrast)
